...
public class VariablesMap {
    ...
    public boolean isInQuestionGrid(String questionName){
        return variables.values().stream()
                ...
                .anyMatch(variable -> // (FILTER_RESULT variables are upper case)
                        (variable.getQuestionItemName().equals(questionName)
                                || variable.getQuestionItemName().equalsIgnoreCase(questionName)));
    }

    public Group getQuestionGridGroup(String questionName) {
        return variables.values().stream()
                .filter(Variable::isInQuestionGrid)
                .filter(variable -> variable.getQuestionItemName().equals(questionName))
                .map(Variable::getGroup)
                .findFirst().orElse(null);
    }
