Feature: Do we extract data ?
  ...
  Scenario Outline: External data extraction
    Given We have data in directory "<Directory>"
    When We create survey unit models from file "<FileName>" with DDI "<DDIFileName>"
    Then We should have <ExpectedNumberOfValues> values for external variable "<ExternalVariableName>" for survey unit "<InterrogationId>"
    And For external variable "<ExternalVariableName>" in survey unit "<InterrogationId>" we should have "<ExpectedValue>" as value number <ExpectedValueIndex>
    Examples:
      | Directory              | FileName                                                          | DDIFileName                   | ExpectedNumberOfValues | ExternalVariableName | InterrogationId | ExpectedValue    | ExpectedValueIndex |
